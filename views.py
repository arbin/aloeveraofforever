...
food_calories = {'1000 ISLAND, SALAD DRSNG,LOCAL1 TBSP':[2,25,2,0,2,15,0.2],
                ...
                '40% BRAN FLAKES, KELLOGGS    1 OZ':[1,90,22,4,0,28.35,0.1],
                ...
                'CAPN CRUNCH CEREAL   1 OZ':[3,120,23,1,0,28.35,1.7],
                ...
                'CORN FLAKES, KELLOGGS 1 OZ':[0,110,24,2,0,28.35,0],
                ...
                'DEVILS FOOD CAKE,CHOCFRST,FMX1 CAKE':[136,3755,645,49,598,1107,55.6],
                'DEVILS FOOD CAKE,CHOCFRST,FMX1 CUPCAK':[4,120,20,2,19,35,1.8],
                'DEVILS FOOD CAKE,CHOCFRST,FMX1 PIECE':[8,235,40,3,37,69,3.5],
                ...
                'PEARS, RAW, DANJOU 1 PEAR':[1,120,30,1,0,200,0],
                ...
                'RAISIN BRAN, KELLOGGS 1 OZ':[1,90,21,3,0,28.35,0.1],
                ...
                'YOGURT, W/ LOFAT MILK,FRUITFLV 8 OZ':[2,230,43,10,10,227,1.6],
...
def opportunity(request):
    return TemplateResponse(request, 'opportunity.html', None)

def benefits(request):
    return TemplateResponse(request, 'benefits-of-aloe.html', None)

def uses(request):
    return TemplateResponse(request, 'aloe-vera-uses.html', None)

def stories(request):
    return TemplateResponse(request, 'stories.html', None)
...
def products(request):
    ...
    geo = Geocoder("AIzaSyDB8YB1Ww_VC5j52_QQTzDityPXKCfHRy8")
    results = geo.reverse_geocode(lat, lng)
    ...
    flplanguage = {'Australia':'en', 'Belgium':'nl', 'Canada':'es',
                    'Czech':'cs', 'Greece':'el', 'Ireland':'en', 'Luxembourg':'de',
                    'Netherlands':'nl', 'New Zealand':'en', 'Northern Ireland':'en',
                    'Philippines':'en', 'Portugal':'pt', 'Slovakia':'sk', 'South Africa':'es',
                    'Spain':'es', 'United Kingdom':'en', 'United States':'en'}

    countrycode = flpcountries[country]
    countrylang = flplanguage[country]

    currentcountry = "Your current country is " + country

    return render_to_response('forever-living-products.html', dict(
                countrycode = countrycode,
                countrylang = countrylang,
                countryofresidence = currentcountry,
                ), context_instance=RequestContext(request))
...
def bmi(request):
    ...
    if request.method == 'POST':
        ...
        if weight <= 0 or weight > 500:
            result = 'Weight cannot be less than 0 or greater than 500'
        elif height <= 0:
            result = 'Height cannot be less than 0'

        else:
            bmi = (weight / (height * height)) * 703.0
            result = 'My BMI is %.2f' % bmi
            if bmi <= 18.5:
                remarks = 'and My Weight Status is Underweight '
            elif bmi >= 18.5 and bmi <= 24.9:
                remarks = 'and My Weight Status is Normal weight '
            elif bmi >= 25 and bmi <= 29.9:
                remarks = 'and My Weight Status is Overweight '
            elif bmi >= 30:
                remarks = 'and My Weight Status is Obese '
        return render_to_response('bmi-result.html', dict(
                bmi = (result + " " + remarks),
                titlerem = (result + ' ' + remarks),
                ), context_instance=RequestContext(request))
    else:
        return render_to_response('bmi.html', dict(
                bmi = "",
                ), context_instance=RequestContext(request))
...
def calorie_burn(request):
    if request.method == 'POST':
        ...
        if weight <= 0 or weight > 500:
            ...
            result = 'Miles cannot be less than 0'

        else:
            calorieburn = ((weight * 0.75) * miles)
            result = 'Calories burned is %.2f' % calorieburn

        return render_to_response('calorie-burn.html', dict(
                calorieburn = result,
                ), context_instance=RequestContext(request))
    else:
        return render_to_response('calorie-burn.html', dict(
...
def calorie_counter(request):
    if request.method == 'POST':
        ...
        return render_to_response('calories-counter-result.html', dict(
                caloriegraph = str(caloriegraph),
                foodname = "CALORIES IN " + food,
                titlerem = food.title()
                ), context_instance=RequestContext(request))
    else:
        return render_to_response('calories-counter.html', dict(
                caloriegraph = '',
                foodname = ''
                ), context_instance=RequestContext(request))
...
def bmr(request):
    ...
    if request.method == 'POST':
        ...
        if age <= 0 or age > 150:
            result = 'Age cannot be less than 0 or greater than 150'
        ...
        remarks = 'My Estimated BMR is ' + str(result) + ' calories/day*'

        return render_to_response('bmr-result.html', dict(
                bmr = remarks,
                titlerem = remarks,
                footnote = "*BMR based on the Mifflin - St. Jeor equations.\
                            Please remember that even the best BMR calculators provide \
                            only a best guess and should be used as a guide only."
                ), context_instance=RequestContext(request))
    else:
        return render_to_response('bmr.html', dict(
                bmr = remarks,
                footnote = "",
                ), context_instance=RequestContext(request))
...
def calories(request):
    if request.method == 'POST':
        ...
        if weight <= 0 or weight > 500:
            ...
            result = 'Age cannot be less than 0'
            ...
            """
            if sex == 'men':
                aw = ((age * 0.2017) - (weight * 0.09036))
                awh = aw + (heartrate * 0.6309) - 55.0969
                t = (duration / 4.184)
                calorieburn = awh * t
            if sex == 'women':
                aw = (age * 0.074) - (weight * 0.05741)
                awh = aw + (heartrate * 0.4472) - 20.4022
                t = (duration / 4.184)
                calorieburn =  awh * t

            result = 'I Burned %.2f Calories ' % calorieburn + 'For %s' % exercise

        return render_to_response('calories-result.html', dict(
                caloriesburned = result,
                titlerem = result,
                ), context_instance=RequestContext(request))
    else:
        return render_to_response('calories.html', dict(
                caloriesburned = "",
                ), context_instance=RequestContext(request))
...
def cost_of_smoking(request):
    if request.method == 'POST':
        ...
        """ Formula:
            ...
            M = Number of cigarettes smoked per day
            C = Number of cigarettes in a pack
            P = Price per pack($/Rs)
        """


        monthly = ((perday / pack )* 30 )* price
        yearly = ((perday / pack ) * 365 ) * price

        return render_to_response('cost-of-smoking.html', dict(
                monthlyexpense = 'Monthly Expense: %.2f' % monthly,
                yearlyexpense = 'Yearly Expense: %.2f' % yearly,
                ), context_instance=RequestContext(request))
    else:
        return render_to_response('cost-of-smoking.html', dict(
                costofsmoking = '',
...
def ww_food_points(request):
    if request.method == 'POST':
        ...
        wwfoodpoints = ( calories / 50 ) + ( fat / 12 ) - ( min([fiber,4]) / 5 )


        return render_to_response('ww-food-points.html', dict(
                wwfoodpoints = 'WW food points: %.2f' % wwfoodpoints,
                ), context_instance=RequestContext(request))
    else:
        return render_to_response('ww-food-points.html', dict(
                wwfoodpoints = '',
...
def pregnancy_diabetes(request):
    if request.method == 'POST':
        height = float(request.POST['height'])
        weight = float(request.POST['weight'])
        age = float(request.POST['age'])
        factor = float(request.POST['exerciselevel'])
        ...
        ibw = 45.5+(((height*0.393700787)-60)*2.3)
        abw = ibw + (0.4*(weight - ibw))
        bmr = 655.1 + (9.6 * weight) + (1.8 * height) - (4.7 * age)
        neededcalories = bmr * factor
        if bmi < 18.5:
            weightgain = '28-40 lbs'
        elif (bmi >= 18.5 and bmi <= 24.9):
            weightgain = '25-35 lbs'
        elif (bmi >= 25.0 and bmi <= 29.9):
            weightgain = '15-25 lbs'
        elif bmi > 30:
            weightgain = '11-20 lbs'

        return render_to_response('pregnancy-diabetes.html', dict(
                bmi = 'Body Mass Index (BMI): %.2f' % bmi,
                ibw = 'Ideal Body Weight (IBW): %.2f' % ibw,
                abw = 'Adjusted Body Weight (ABW): %.2f' % abw,
                bmr = 'Basal Metabolic Rate (BMR): %.2f' % bmr,
                weightgain = 'Weight Gain: ' + weightgain,
                dcalories = 'Calories: %.2f' % neededcalories
                ), context_instance=RequestContext(request))
    else:
        return render_to_response('pregnancy-diabetes.html', dict(
                bmi = '',
